...
func deleteCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Delete a user [DEPRECATED]",
		Long: heredoc.Doc(`
		...
		`),
		Deprecated: "Use delete-users command instead",
		RunE: func(cmd *cobra.Command, args []string) error {
			c, err := config.GetConfig(cmd).GetClient(cmd)
			if err != nil {
				return err
...
func deleteMultipleCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "delete-users --new-channel-user-id [user-id] --hard-delete-users [true|false] --hard-delete-messages [true|false] --hard-delete-conversations [user-id1] [user-id2] ...",
		Short: "Delete multiple users",
		Long: heredoc.Doc(`
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			usersHardDel, _ := cmd.Flags().GetBool("hard-delete-users")
			msgHardDel, _ := cmd.Flags().GetBool("hard-delete-messages")
			convHardDel, _ := cmd.Flags().GetBool("hard-delete-conversations")
			newChannelOwnerID, _ := cmd.Flags().GetString("new-channel-owner-id")

			task, err := c.DeleteUsers(cmd.Context(), args, stream.DeleteUserOptions{
				User:              getDeleteType(usersHardDel),
				Messages:          getDeleteType(msgHardDel),
				Conversations:     getDeleteType(convHardDel),
	...
	fl := cmd.Flags()
	fl.Bool("hard-delete-users", false, "[optional] Hard delete everything related to the users")
	fl.Bool("hard-delete-messages", false, "[optional] Hard delete all messages related to the users")
	fl.Bool("hard-delete-conversations", false, "[optional] Hard delete all conversations related to the users")
	fl.String("new-channel-owner-id", "", "[optional] Channels owned by hard-deleted users will be transferred to this userID")
