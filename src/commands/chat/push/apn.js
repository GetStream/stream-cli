		...
		try {
			...
			if (flags.disable) {
				...
				if (result.proceed) {
					...
					this.log(`Push notifications have been ${chalk.red('disabled')} with ${chalk.bold('APN')}.`);
				}
				this.exit();
				...
				const type = await prompt([
					{
						...
						required: true,
						choices: [ { message: 'certificate', value: 'cert' }, { message: 'token', value: 'token' } ]
					}
				]);
			...
			const payload = {
				apn_config: {
					p12_cert: fs.existsSync(flags.p12_cert) ? fs.readFileSync(flags.p12_cert) : '',
					auth_key: fs.existsSync(flags.auth_key) ? fs.readFileSync(flags.auth_key, 'utf-8') : '',
					key_id: flags.key_id || '',
			...
			this.log(`Push notifications have been ${chalk.green('enabled')} with ${chalk.bold('APN')}.`);
			this.exit();
		} catch (error) {
